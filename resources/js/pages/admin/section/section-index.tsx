...
export default function SectionIndex({ sections, divisions }: SectionIndexProps) {
    ...
    const formDefaults = { name: '', acronym: '', code: '', division_id: '' };
...
const SectionContent = ({ sections, divisions }: { sections: Section[]; divisions: Division[] }) => {
    ...
    const filteredSections = sections.filter((section) => selectedDivisions.length === 0 || selectedDivisions.includes(section.division_id));
    ...
    return (
        <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <SectionTable sections={filteredSections} search={search} setSearch={setSearch} />
...
const SectionTable = ({ sections, search, setSearch }: { sections: Section[]; search: string; setSearch: Dispatch<SetStateAction<string>> }) => {
