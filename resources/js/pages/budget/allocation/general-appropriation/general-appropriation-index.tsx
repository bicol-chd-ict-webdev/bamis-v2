...
import { GENERAL_APPROPRIATION_FORM_DEFAULTS } from '@/constants/form-defaults';
...
import { Coins, PencilLine, Plus, RefreshCwIcon, Trash2, View, X } from 'lucide-react';
...
import ViewGeneralAppropriation from './modals/view-general-appropriation';
...
}: GeneralAppropriationIndexProps): JSX.Element {
    return (
        <ModalProvider<Allocation> formDefaults={GENERAL_APPROPRIATION_FORM_DEFAULTS}>
            <LoadingProvider>
                <SearchProvider>
...
const GeneralAppropriationContent = (): JSX.Element => {
    ...
    return (
        <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            ...
            <GeneralAppropriationTable filteredAllocations={filteredAllocations} />
            <Modals />
        </div>
    );
};

const GeneralAppropriationTable = ({ filteredAllocations }: { filteredAllocations: Allocation[] }): JSX.Element => {
    const { handleOpenModal } = useModalContext<Allocation>();
    ...
    const dropdownItems = useMemo(
        () => [
            ...
            },
            {
                ...
                label: 'View',
                action: 'view',
                handler: (row: any): void => router.get(budget.generalAppropriations.show(row.original).url),
            },
            {
                icon: <PencilLine />,
                label: 'Edit',
    ...
    if (allocations.length === 0 && !isLoading) {
        return (
            <EmptyState
                ...
                description="Record the authorized amounts from the GAA to create a baseline for obligations and disbursements."
            />
        );
    }
...
const Modals = memo((): JSX.Element => {
    ...
    return (
        <>
            {modal === 'create' && <CreateGeneralAppropriation openModal={true} closeModal={handleCloseModal} />}
            {modal === 'edit' && <EditGeneralAppropriation openModal={true} closeModal={handleCloseModal} />}
            {modal === 'delete' && <DeleteGeneralAppropriation openModal={true} closeModal={handleCloseModal} />}
            {modal === 'view' && <ViewGeneralAppropriation openModal={true} closeModal={handleCloseModal} />}
        </>
    );
});
