...
final class SpecialAllotmentController extends Controller
{
    ...
    public function show(Allocation $special_allotment): Response
    {
        return Inertia::render('budget/allocation/show-allocation', [
            'allocation' => $special_allotment,
            ...
            'expenditures' => fn (): array => ExpenditureResource::collection(
                $this->expenditureRepository->comboboxList($special_allotment->allotment_class_id)
            )->resolve(),
            'objectDistributions' => fn (): array => ObjectDistributionResource::collection(
                $this->objectDistributionRepository->list($special_allotment->id)
            )->resolve(),
            'officeAllotments' => $this->officeAllotmentRepository->list((int) $special_allotment->id),
            'officeAllotmentsGroupedBySection' => $this->officeAllotmentRepository->listGroupedBySection((int) $special_allotment->id),
            'obligations' => fn (): array => ObligationResource::collection(
                $this->obligationRepository->list((int) $special_allotment->id)
            )->resolve(),
    ...
    public function destroy(Allocation $special_allotment, DestroyAllocation $action): RedirectResponse
    {
